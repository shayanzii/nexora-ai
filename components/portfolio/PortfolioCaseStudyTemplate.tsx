      ...
      <main id="main-content">
        <section className="relative overflow-hidden px-6 py-16 lg:px-8 lg:py-20">
          ...
          <div className="mx-auto max-w-7xl">
            <Link href="/portfolio" className="nexora-nav-link mb-8 inline-flex items-center gap-2 text-sm">
              ← Back to Portfolio
            </Link>
            {project.isIllustrative ? (
              <p className="mb-4 inline-flex rounded-full border border-nexora-border bg-nexora-surface px-3 py-1 text-xs font-medium text-nexora-muted">
                Illustrative project — representative outcomes for this industry
              </p>
            ) : null}
          ...
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_320px] lg:gap-14">
            <div>
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="nexora-card rounded-2xl border-nexora-border p-5">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-nexora-muted">Before</p>
                  <p className="mt-3 text-sm leading-7 text-nexora-text">{project.before}</p>
                </div>
                <div className="nexora-card rounded-2xl border-nexora-primary/30 bg-nexora-primary/5 p-5">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-nexora-hover">After</p>
                  <p className="mt-3 text-sm leading-7 text-nexora-text">{project.after}</p>
                </div>
              </div>

              <div className="mt-12">
                <p className="nexora-eyebrow">The Problem</p>
                <p className="mt-4 text-lg leading-8 text-nexora-muted">{project.challenge}</p>
              </div>

              <div className="mt-12">
                <p className="nexora-eyebrow">What We Built</p>
            ...
            <aside className="nexora-card h-fit rounded-3xl p-6 lg:sticky lg:top-24">
              <p className="nexora-eyebrow text-sm">Industry</p>
              <p className="mt-2 font-semibold text-nexora-text">{project.industry}</p>
              <p className="nexora-eyebrow mt-6 text-sm">Tools Used</p>
              <ul className="mt-3 space-y-2">
                {project.technologies.map((tech) => (
                  <li key={tech} className="nexora-surface rounded-xl px-3 py-2 text-sm text-nexora-text">
                    {tech}
                  </li>
                ))}
              </ul>
              <BookConsultationButton className="mt-6 w-full rounded-full px-5 py-3 text-sm shadow-[0_0_24px_rgba(185,28,28,0.28)]">
        ...
        <section className="nexora-section px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="max-w-2xl">
              <p className="nexora-eyebrow">Business Outcomes</p>
              <h2 className="nexora-heading-section mt-3">Results that matter to the bottom line.</h2>
            </div>
        ...
        <section className="px-6 pb-24 lg:px-8 lg:pb-32">
          <div className="nexora-glow nexora-card mx-auto max-w-7xl rounded-[2rem] border-nexora-primary/25 p-8 shadow-[0_0_80px_rgba(185,28,28,0.1)] lg:p-12">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <h2 className="nexora-heading-section mt-3">Want similar results for your business?</h2>
              <p className="mt-5 text-lg leading-8 text-nexora-muted">
                Book a free call. We&apos;ll analyze your business and show you exactly what AI could do—no obligation.
              </p>
